...
pthread_mutex_t carLock;	//Mutex or lock for cars passing onto the bridge
...
int rushHour = 0;		//Rush Hour streak
...
{
	if((rushHour < 6) && (br.num_car == 0) && (direc == EAST) && (nextE < counterE) && (vid == eastWait[nextE])){
		//No Cars on bridge, Direction = EAST, vid = first waiting position in East array
		nextE++;		//increment nextE
		br.safe = 1;		//bridge is safe
		return 1;
	}
	else if((rushHour < 6)&&(br.num_car == 0) && (direc == WEST) && (nextW < counterW) && (vid == westWait[nextW])){
		//No cars on bridge, Direction = WEST, next < size of count, vid = first waiting position in West array
		nextW++;
		br.safe = 1;
		return 1;
	}
	else if((rushHour < 6)&&(br.num_car < MAX_BR_CAP) && (br.curr_dir == EAST) && (nextE < counterE) && (vid == eastWait[nextE])){
		//Number of cars on bridge less than limit and in same direction
		nextE++;
		br.safe = 1;		//bridge is safe
		return 1;
	}
	else if((rushHour < 6)&&(br.num_car < MAX_BR_CAP) && (br.curr_dir == WEST) && (nextW < counterW) && (vid == westWait[nextW])){
		nextW++;
		br.safe = 1;
		return 1;
	}
...
void ArriveBridge(int vid, int direc)
{
	...
	rushHour++;			//Increment Rush Hour
	
	if((br.dirWait[direc] > 0) && (br.num_car < MAX_BR_CAP) && (rushHour < 6)){
