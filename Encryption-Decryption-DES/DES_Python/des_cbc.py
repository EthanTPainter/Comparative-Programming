...
xorBlock = build8Byte(sys.argv[1])

des = DES.new(key, DES.MODE_ECB)	

start = time.time()

#read 64 bit blocks at a time and encrypt or decrypt in cbc mode and write results to file output
for readBlock in iter(lambda: fin.read(8), ""):
	if(len(readBlock) < 8):
		readBlock += ' ' * (8 - len(readBlock))
	if(encryptOrDecrypt == 0):
		writeBlock = des.decrypt(readBlock)
		writeBlock = xor_strings(writeBlock, xorBlock)
		xorBlock = readBlock

	if(encryptOrDecrypt == 1):
		writeBlock = xor_strings(readBlock,xorBlock)
		writeBlock = des.encrypt(writeBlock)
		xorBlock = writeBlock		
